...
2. Run `pnpm build:registry` to generate `meta.yml`, all `meta.<locale>.yml`
   files, and all resume files for the supported locale languages, then
   regenerate `src/registry.json`.
...
```bash
OPENAI_API_KEY=sk-... pnpm build:registry
```

If you only want to rebuild `src/registry.json` from existing files without
generating anything, use the `--registry-only` flag:

```bash
pnpm build:registry --registry-only
```

To force regeneration of all metadata and resumes, use the `--force` flag:

```bash
OPENAI_API_KEY=sk-... pnpm build:registry --force
```
